...
현재 README의 'CLI·변이 검사·이력이 아직 없다'는 문장은 위 소스와 달라 바로잡았다. 새 실행 기능을 추가한 것은 아니다. 전체 진행 상태는 [통합 실행 계획](https://github.com/hwain-ai/SENTINEL/blob/main/docs/exec-plans/active/2026-09-sentinel-unified-entry.md)에 있다.
...
- 원래 JDK의 내부 링크와 공통 봉인 입력의 링크 금지 조건을 함께 만족해야 한다. [의존성 검증기](../scripts/dependency_lock.py)의 폴더 권한0700과 [공통 설치기](https://github.com/hwain-ai/SENTINEL/blob/main/src/sentinel/content_root.py)의 봉인 권한0500도 그대로는 맞지 않는다. 읽기 전용 mount를 해제하자는 뜻은 아니다.
